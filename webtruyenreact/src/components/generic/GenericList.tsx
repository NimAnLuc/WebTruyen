...
const GenericList = <T extends { id: number; status?: number }>({
  config,
  lookupData = {},
}: GenericListProps<T>) => {
  ...
  if (lastPage <= maxPagesToShow) {
    // Hiển thị tất cả trang nếu lastPage <= 8
    for (let i = 1; i <= lastPage; i++) {
      pageNumbers.push(i);
    }
  } else {
    // Hiển thị tối đa 8 trang: 1, 2, page-2, page-1, page, page+1, page+2, ..., lastPage
    const leftSibling = Math.max(3, page - 2); // Bắt đầu từ trang 3 hoặc page-2
    const rightSibling = Math.min(lastPage - 1, page + 2); // Kết thúc trước lastPage hoặc page+2
          ...
          <div className="card shadow rounded">
            <div className="card-body">
              ...
              <div className="mb-4 d-flex gap-2">
                <Link to={`/admin/${config.entityName.toLowerCase()}/add`}>
                  <button className="btn btn-success">
                    <FaPlus className="me-1" /> Thêm
                  </button>
                </Link>
                <Link to={`/admin/${config.entityName.toLowerCase()}/trash`}>
                      ...
                      {sortedData.length > 0 ? (
                        ...
                        <tr>
                          ...
                          >
                            ...
                            <p className="text-sm">
                              Hãy thêm {config.entityName.toLowerCase()} mới để
                              hiển thị danh sách.
                            </p>
                          </td>
                        </tr>
